...
/**
 * 获取字典列表  sid select的id,dict_type 字典类型,op_val option选中值,callback 回调函数,async 同步异步,is_null是否需要添加空行
 * 调用方法 getDict("Q_USER_TYPE","USER_TYPE","${param.Q_USER_TYPE}",null,false,true);
 */
function getDict(sid,dict_type,op_val,callback,async,is_null){
	//回调函数(执行js)
	var $callback = callback;
	if (! $.isFunction($callback)){
		$callback = eval('(' + callback + ')');
	...
	$.ajax({
		type: "POST",
	    url: _contextPath+'/topic/ajax/getDict?ss='+timestamp,
	    data: {"TYPE" : dict_type},
	    dataType: "json",
	    async: $async,
		success: function(json){
			if(json!=null && json != ""){
				var data = json.resultObj;
				...
				for(var i in data){
					// 添加option
					oSelect.options.add(new Option(data[i].DICT_NAME, data[i].DICT_VALUE));
				}
				//option回填
				for(var i=0; i < oSelect.options.length; i++){
					if(oSelect.options[i].value == op_val){  
